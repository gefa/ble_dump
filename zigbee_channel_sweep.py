...
from grc.gr_ble_zig import gr_ble_zig as gr_block
...
def init_opts(gr):
  ...
  ble.add_option("-c", "--current_ble_channels", type="string", default='11,17,21,26', help="BLE channels to scan [default=%default]")
...
if __name__ == '__main__':
  ...
  print('Capturing on channel [ {:d} ] @ {:d} MHz'.format(current_ble_chan, int(gr_block.get_freq() / 1000000)))
  ...
  try:
    while True:
      ...
      if datetime.now() >= hopping_time:
        ...
        hopping_time = datetime.now() + timedelta(seconds=opts.ble_scan_window)
        current_hop +=1
        print('Switching to channel [ {:d} ] @ {:d} MHz'.format(current_ble_chan, int(gr_block.get_freq() / 1000000)))
